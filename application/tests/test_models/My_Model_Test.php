...
class My_Model_Test extends CITestCase {
    ...
    public function test_check_table_exists() {
        if ($this->CI->db->query("SELECT name FROM sqlite_master WHERE name='blog'")
                         ->num_rows() == 0
        ) {
            $return = false;
        } else {
    ...
    public function test_check_ele_model() {
        ...
        $this->assertTrue(is_object($this->CI->blog->where('blog_title')
                                                   ->get()));
        $id = $this->CI->blog->insert(['blog_title' => 'avenirer']);
        $this->assertGreaterThan(0, $id);
        $this->CI->blog->update(['blog_title' => 'check'], $id);
