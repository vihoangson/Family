...
<div class="row autoheight">
	...
	foreach ((array)$data as $key => $value) {
		?>
		<div class='col-sm-3'>
			<a data-id="<?= $key; ?>" href="javascript:void(0)" class="thumbnail open-modal"><img src="<?= ($value?$value:"/asset/data/no_image.png"); ?>"></a>
		</div>
		<?php
	}
	?>
...
<script>
	$(".add-more").click(function(event) {
		...
		$(this).before(cln);
		$.post('/ajax/do_ajax/save_img_instant', {id: $(".open-modal").length+1,src:""}, function(data, textStatus, xhr) {
			location.reload();
		});
	});
	$(".open-modal").open_media({
		...
		callbackevent_before:function(selector_this){
			$(document).on("click","#modal-upload-media .modal-body img",function(){
				...
				$.post('/ajax/do_ajax/save_img_instant', {id: $(".current_img").data('id'),src:$(this).attr("src")}, function(data, textStatus, xhr) {
					console.log(data);
				});
