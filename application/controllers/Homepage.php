...
class Homepage extends MY_Controller {
    ...
    private function getDataHomepage($condition = null) {
        ...
        foreach ($kn as $key => $value) {
            $rs                  = $this->db->where("kyniem_id", $value->id)
                                            ...
                                            ->join("user", "user.id=comment_user")
                                            ->order_by("id", "desc")
                                            ->get('comment')
                                            ->result();
            $comment[$value->id] = $rs;
    ...
    public function login($case = null) {
        ...
        if ($this->input->post('username') && $this->input->post('password')) {
            ...
            switch ($username) {
                case "bo":
                    $username = $this->db->where('id', 11)
                                         ->get("user")
                                         ->row()->username;
                break;
                case "me":
                    $username = $this->db->where('id', 12)
                                         ->get("user")
                                         ->row()->username;
                break;
            }
    ...
    public function ajax_delete_img() {
        ...
        try {
            ...
            $rs = json_decode($this->db->get('kyniem', 1)
                                       ->row()->kyniem_images, true);
    ...
    public function ajax_delete_comment() {
        $id     = $this->input->post('id');
        $string = json_encode($this->db->where("id", $id)
                                       ->get("comment")
                                       ->result());
    ...
    public function fb_callback() {
        ...
        if (!$this->session->userdata('fb_access_token')) {
            ...
            if (true) {
                $rs_fb = $this->db->where("archive_key", "lg_fb")
                                  ->get('archive')
                                  ->row()->archive_content;
