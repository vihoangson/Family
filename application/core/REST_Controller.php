...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest') {
        ...
        if ($this->request->format && $this->request->body) {
            $this->request->body = $this->format->factory($this->request->body, $this->request->format)
                                                ->to_array();
        ...
        if (!$this->input->is_ajax_request() && config_item('rest_ajax_only')) {
            $response = [config_item('rest_status_field_name')  => false,
                         config_item('rest_message_field_name') => 'Only AJAX requests are accepted.'
            ];
    ...
    public function _remap($object_called, $arguments) {
        ...
        if (config_item('force_https') && !$this->_detect_ssl()) {
            $this->response([config_item('rest_status_field_name')  => false,
                             config_item('rest_message_field_name') => 'Unsupported protocol'
            ], 403);
        }
        ...
        if (config_item('rest_enable_keys') && $use_key && $this->_allow === false) {
            ...
            $this->response([config_item('rest_status_field_name')  => false,
                             config_item('rest_message_field_name') => 'Invalid API Key ' . $this->rest->key
            ], 403);
        }
        ...
        if (config_item('rest_enable_keys') && $use_key && !empty($this->rest->key) && !$this->_check_access()) {
            ...
            $this->response([config_item('rest_status_field_name')  => false,
                             config_item('rest_message_field_name') => 'This API key does not have access to the requested controller.'
            ], 401);
        }
        ...
        if (!method_exists($this, $controller_method)) {
            $this->response([config_item('rest_status_field_name')  => false,
                             config_item('rest_message_field_name') => 'Unknown method.'
            ], 404);
        }
        ...
        if (config_item('rest_enable_keys') && !empty($this->rest->key)) {
            ...
            if (config_item('rest_enable_limits') && !$this->_check_limit($controller_method)) {
                $response = [config_item('rest_status_field_name')  => false,
                             config_item('rest_message_field_name') => 'This API key has reached the hourly limit for this method.'
                ];
                $this->response($response, 401);
            }

            // If no level is set use 0, they probably aren't using permissions
            $level = isset($this->methods[$controller_method]['level']) ? $this->methods[$controller_method]['level'] :
                0;
            ...
            $response = [config_item('rest_status_field_name')  => false,
                         config_item('rest_message_field_name') => 'This API key does not have enough permissions.'
            ];
    ...
    public function response($data = null, $http_code = null, $continue = false) {
        ...
        elseif ($data === null && is_numeric($http_code)) {
            ...
            elseif (method_exists($this->format, 'to_' . $this->response->format)) {
                ...
                $output = $this->format->factory($data)
                                       ->{'to_' . $this->response->format}();
    ...
    protected function _detect_api_key() {
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] :
            ...
            if (!($row = $this->rest->db->where(config_item('rest_key_column'), $key)
    ...
    protected function _log_request($authorized = false) {
        $status = $this->rest->db->insert(config_item('rest_logs_table'), [
            ...
            'params'     => $this->_args ?
                (config_item('rest_logs_json_params') ? json_encode($this->_args) : serialize($this->_args)) : null,
    ...
    protected function _check_limit($controller_method) {
        ...
        $result = $this->rest->db->where('uri', $this->uri->uri_string())
        ...
        elseif ($result->hour_started < time() - (60 * 60)) {
            ...
            $this->rest->db->where('uri', $this->uri->uri_string())
    ...
    public function options($key = null, $xss_clean = true) {
        ...
        return array_key_exists($key, $this->_options_args) ?
            $this->_xss_clean($this->_options_args[$key], $xss_clean) : false;
    }
    ...
    public function post($key = null, $xss_clean = true) {
        ...
        return array_key_exists($key, $this->_post_args) ? $this->_xss_clean($this->_post_args[$key], $xss_clean) :
            false;
    }
    ...
    public function delete($key = null, $xss_clean = true) {
        ...
        return array_key_exists($key, $this->_delete_args) ? $this->_xss_clean($this->_delete_args[$key], $xss_clean) :
            false;
    }
    ...
    public function patch($key = null, $xss_clean = true) {
        ...
        return array_key_exists($key, $this->_patch_args) ? $this->_xss_clean($this->_patch_args[$key], $xss_clean) :
            false;
    }
    ...
    protected function _check_login($username = '', $password = false) {
        ...
        if (!$this->config->item('auth_source') && $rest_auth == 'digest') { // for digest we do not have a password passed as argument
            return md5($username . ':' . $this->config->item('rest_realm') . ':' . (isset($valid_logins[$username]) ?
                    $valid_logins[$username] : ''));
        }
    ...
    protected function _prepare_digest_auth() {
        ...
        if ($digest['response'] != $valid_response) {
            $this->response([config_item('rest_status_field_name')  => 0,
    ...
    protected function _check_whitelist_auth() {
        ...
        if (!in_array($this->input->ip_address(), $whitelist)) {
            $this->response([config_item('rest_status_field_name')  => false,
                             config_item('rest_message_field_name') => 'IP not authorized'
            ], 401);
        }
    }
