...
/*
 * Github: https://github.com/avenirer/CodeIgniter-MY_Model
 ...
 *              If set to TRUE tells MY_Model that the table has 'created_at','updated_at' (and 'deleted_at' if
 *              $this->soft_delete is set to TRUE) If given an array as parameter, it tells MY_Model, that the first
 *              element is a created_at field type, the second element is a updated_at field type (and the third
 *              element is a deleted_at field type)
 *          $this->soft_deletes = FALSE;
 *              Enables (TRUE) or disables (FALSE) the "soft delete" on records. Default is FALSE
 *          $this->timestamps_format = 'Y-m-d H:i:s'
 *              You can at any time change the way the timestamp is created (the default is the MySQL standard datetime
 *              format) by modifying this variable. You can choose between whatever format is acceptable by the php
 *              function date() (default is 'Y-m-d H:i:s'), or 'timestamp' (UNIX timestamp)
 ...
 *          array('Address_model','foreign_key','another_local_key'); Allows establishing ONE TO ONE or more ONE TO ONE
 *          relationship(s) between models/tables
 *          $this->has_many['posts'] = 'Post_model' or $this->has_many['posts'] =
 *          array('Posts_model','foreign_key','another_local_key'); Allows establishing ONE TO MANY or more ONE TO MANY
 *          relationship(s) between models/tables
 *          $this->has_many_pivot['posts'] = 'Post_model' or $this->has_many_pivot['posts'] =
 *          array('Posts_model','foreign_primary_key','local_primary_key'); Allows establishing MANY TO MANY or more
 *          MANY TO MANY relationship(s) between models/tables with the use of a PIVOT TABLE
 *              !ATTENTION: The pivot table name must be composed of the two table names separated by "_" the table
 *              names having to to be alphabetically ordered (NOT users_posts, but posts_users). Also the pivot table
 *              must contain as identifying columns the columns named by convention as follows: table_name_singular + _
 *              + foreign_table_primary_key. For example: considering that a post can have multiple authors, a pivot
 *              table that connects two tables (users and posts) must be named posts_users and must have post_id and
 *              user_id as identifying columns for the posts.id and users.id tables.
 ...
 *              time use the MY_Model's caching. By default, MY_Model uses the files to cache result. If you want to
 *              change the way it stores the cache, you can change the $cache_driver property to whatever CodeIgniter
 *              cache driver you want to use. Also, with $cache_prefix, you can prefix the name of the caches. by
 *              default any cache made by MY_Model starts with 'mm' + _ + "name chosen for cache"
 ...
 *              $this->delete_cache_on_save by setting it to TRUE. If set to TRUE the model will auto-delete all cache
 *              related to the model's table whenever you write/update/delete data from that table.
...
class MY_Model extends CI_Model {
    ...
    public function __construct() {
        ...
        $this->_fetch_table();
        $this->pagination_delimiters = (isset($this->pagination_delimiters)) ? $this->pagination_delimiters :
            ['<span>', '</span>'];
        ...
        if ($this->form_validation->run()) {
            ...
            if (!empty($row_fields_to_update)) {
                foreach ($row_fields_to_update as $key => $field) {
                    ...
                    } elseif (in_array($key, $this->table_fields)) {
     ...
     * Inserts data into table. Can receive an array or a multidimensional array depending on what kind of insert we're
     * talking about.
     *
     * @param $data
     *
    ...
    public function update($data = null, $column_name_where = null, $escape = true) {
        ...
        if ($multi === false) {
            ...
            if ($escape) {
                ...
                if ($this->_database->set($data, null, false)
                                    ->update($this->table)
                ) {
                    $this->_prep_after_write();
                    $affected = $this->_database->affected_rows();
            ...
            foreach ($data as $row) {
                ...
                if ($escape) {
                    ...
                    if ($this->_database->set($row, null, false)
                                        ->update($this->table)
                    ) {
                        $rows++;
                    }
    ...
    public function restore($where = null) {
        ...
        if ($affected_rows = $this->_database->update($this->table, [$this->_deleted_at_field => null])) {
            $this->_prep_after_write();
     ...
     * allows the user to retrieve records from other interconnected tables depending on the relations defined before
     * the constructor
     *
            ...
            if (!isset($pivot_table)) {
                ...
                if (!empty($request['parameters'])) {
                    if (array_key_exists('fields', $request['parameters'])) {
                        if ($request['parameters']['fields'] == '*count*') {
                            ...
                            foreach ($fields as $field) {
                                $select[] = (strpos($field, '.') === false) ?
                                    '`' . $foreign_table . '`.`' . trim($field) . '`' : trim($field);
                            }
                    ...
                    if (array_key_exists('where', $request['parameters']) || array_key_exists('non_exclusive_where', $request['parameters'])) {
                        $the_where = array_key_exists('where', $request['parameters']) ? 'where' :
                            'non_exclusive_where';
                    }
                    $sub_results = isset($the_where) ?
                        $sub_results->where($request['parameters'][$the_where], null, null, false, false, true) :
                        $sub_results;
                ...
                $sub_results = $sub_results->where($foreign_key, $local_key_values)
                                           ->get_all();
            } else {
                ...
                if (!empty($order_inside_array)) {
                    $order_inside_str = '';
                    foreach ($order_inside_array as $order_by_inside) {
                        $order_inside_str .= (strpos($order_by_inside[0], ',') === false) ?
                            '`' . $foreign_table . '`.`' . $order_by_inside[0] . ' ' . $order_by_inside[1] :
                            $order_by_inside[0] . ' ' . $order_by_inside[1];
                        $order_inside_str .= ',';
                    }
                    $order_inside_str = rtrim($order_inside_str, ",");
                    $this->_database->order_by(rtrim($order_inside_str, ","));
                }
                $sub_results = $this->_database->get($foreign_table)
                                               ->result_array();
                $this->_database->reset_query();
            }
                ...
                foreach ($sub_results as $result) {
                    ...
                    if (isset($pivot_table)) {
                        ...
                        if (isset($get_relate) and $get_relate === true) {
                            $subs[$the_local_key][$the_foreign_key] = $this->{$relation['foreign_model']}->where($local_key, $result[$local_key])
                                                                                                         ->get();
                        } else {
            ...
            foreach ($options as $option) {
                if (isset($this->{$option}) && !empty($this->{$option})) {
                    foreach ($this->{$option} as $key => $relation) {
                        if (!is_array($relation)) {
                            ...
                            if ($this->_is_assoc($relation)) {
                                ...
                                if ($option == 'has_many_pivot') {
                                    $pivot_table       = $relation['pivot_table'];
                                    $pivot_local_key   = (array_key_exists('pivot_local_key', $relation)) ?
                                        $relation['pivot_local_key'] : $this->table . '_' . $this->primary_key;
                                    $pivot_foreign_key = (array_key_exists('pivot_foreign_key', $relation)) ?
                                        $relation['pivot_foreign_key'] : $foreign_table . '_' . $foreign_key;
                                    $get_relate        = (array_key_exists('get_relate', $relation) && ($relation['get_relate'] === true)) ?
                                        true : false;
                                }
                            } else {
                                ...
                                if ($option == 'has_many_pivot') {
                                    ...
                                    $pivot_foreign_key = $foreign_table . '_' . $foreign_key;
                                    $get_relate        = (isset($relation[3]) && ($relation[3] === TRUE())) ? true :
                                        false;
                                }
                            }
                        ...
                        $this->_relationships[$key] = ['relation'      => $option,
                                                       'relation_key'  => $key,
    ...
    private function _set_timestamps() {
        if ($this->timestamps !== false) {
            $this->_created_at_field = (is_array($this->timestamps) && isset($this->timestamps[0])) ?
                $this->timestamps[0] : 'created_at';
            $this->_updated_at_field = (is_array($this->timestamps) && isset($this->timestamps[1])) ?
                $this->timestamps[1] : 'updated_at';
            $this->_deleted_at_field = (is_array($this->timestamps) && isset($this->timestamps[2])) ?
                $this->timestamps[2] : 'deleted_at';
        }
        ...
        if (isset($total_rows)) {
            if ($total_rows != 0) {
                ...
                for ($i = 1; $i <= $number_of_pages; $i++) {
                    ...
                    $links      .= (($page_number == $i) ? anchor($uri_string, $i) :
                        anchor($uri_string . '/' . $i, $i));
    ...
    private function _build_sorter($data, $field, $order_by, $sort_by = 'DESC') {
        usort($data, function ($a, $b) use ($field, $order_by, $sort_by) {
            ...
            return strtoupper($sort_by) == "DESC" ? ((isset($array_a[$order_by]) && isset($array_b[$order_by])) ?
                ($array_a[$order_by] < $array_b[$order_by]) : -1) :
                ((isset($array_a[$order_by]) && isset($array_b[$order_by])) ?
                    ($array_a[$order_by] > $array_b[$order_by]) : -1);
        });
